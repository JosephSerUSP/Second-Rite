 ...
 */
var ThestraVertexShadingSemantics;
(function (ThestraVertexShadingSemantics) {
    const MODULUS = 65521;
    ...
    ThestraVertexShadingSemantics.hash01 = hash01;
    ...
    ThestraVertexShadingSemantics.valueNoise = valueNoise;
    ...
    ThestraVertexShadingSemantics.fractalNoise = fractalNoise;
    function validateRgb(problems, value, where) {
    ...
    ThestraVertexShadingSemantics.validate = validate;
    function compile(layers, where = 'vertexShadingLayers') {
    ...
    ThestraVertexShadingSemantics.compile = compile;
    ...
    ThestraVertexShadingSemantics.sampleCompiled = sampleCompiled;
    function sample(layers, x, y, target) {
        return sampleCompiled(compile(layers), x, y, target);
    }
    ThestraVertexShadingSemantics.sample = sample;
    function grid(layers, width, height) {
        const compiled = compile(layers);
        const result = [];
        for (let y = 0; y <= height; y++) {
            const row = [];
            for (let x = 0; x <= width; x++)
                row.push(sampleCompiled(compiled, x, y));
            result.push(row);
        }
        return result;
    }
