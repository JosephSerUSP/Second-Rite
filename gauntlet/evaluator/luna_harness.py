...
from gauntlet.evaluator.rubric import RUBRIC_CATEGORIES, ENSEMBLE_EXTRA_CATEGORIES, LUNA_SYSTEM_PROMPT
...
class LunaHarness:
    ...
    def evaluate_round(
        ...
        """
        Submits an evaluation package to gpt-5.6-luna.
        image_paths contains pairs of {"label": description, "path": file_path}
        """
        content_items = [
            {
                "type": "text",
                "text": f"Evaluation target: {character_name.upper()} | Round: {round_name}\n"
                        f"Context and Stage Information:\n{context_prompt}\n\n"
                        f"Please critically examine the attached visual sheets and return your strict adversarial evaluation JSON."
            }
        ]
        ...
        for item in image_paths:
            ...
            content_items.append({
                "type": "text",
                "text": f"--- Sheet: {label} ({os.path.basename(path)}) ---"
            })
            content_items.append({
                "type": "image_url",
                "image_url": {
        ...
        for attempt in range(retries):
            try:
                ...
                numeric_scores = [float(scores[k]) for k in applicable_cats if k in scores]
                ...
                # Ensure strict verdict agreement
