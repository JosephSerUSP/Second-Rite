...
local function applyMature(effectData, a, b, session, context)
    local events = {}
    local ctxElement = context and context.element or nil
    local ctxUser = context and context.user or nil
...
-- Effects publish resolved facts at the mutation site.  This keeps the
-- authority with effect resolution instead of asking a caller to infer and
-- repair state after the mature effect has returned.
local function resolved(events, session)
    for _, ev in ipairs(events or {}) do resolved_event.attach(ev, session) end
    return events
end

local function maxHpEvent(target, transition, extra)
    local ev = { type = "max_hp_change", target = target, before = transition.before,
        after = transition.after, value = transition.delta, hpGranted = transition.hpGranted,
...
local function productionKillContext(a, session, context)
    ...
    local merged, killReactions = {}, {}
    for key, value in pairs(participants) do merged[key] = value end
    for _, participant in ipairs(participants.killReactions or {}) do table.insert(killReactions, participant) end
    table.insert(killReactions, productionKill)
    merged.killReactions = killReactions
    local out = { hpDamageParticipants = merged }
    setmetatable(out, { __index = original, __newindex = function(_, key, value) original[key] = value end })
    return out
end
...
local function applyDamage(effectData, a, b, session, context)
    ...
    return damage_transition.apply({
        effectData = effectData, source = a, target = b, session = session,
        context = transitionContext, calculate = effects.calculateHpDamage,
        commit = effects.commitHpDamage, publish = function(events) return resolved(events, session) end,
...
function effects.apply(effectData, a, b, session, context)
    ...
    if effectData.type == "hp_drain" and a and a ~= b then
        ...
        local transitionContext = productionKillContext(a, session, context)
        local events, kill = effects.applyHpDrain(effectData, a, b, session, transitionContext)
        if kill then
            damage_transition.resolveKill({ source = a, target = b, session = session,
                context = transitionContext, events = events, kill = kill,
                applyEffect = function(nestedEffect, nestedSource, nestedTarget, nestedContext)
                    return effects.apply(nestedEffect, nestedSource, nestedTarget, session, nestedContext)
                end })
        end
        local damageEv, healEv = findEvent(events, "damage", b), findEvent(events, "heal", a)
        if damageEv and healEv then
            a.hp = beforeHp
            local healed, cap = vitality.applyHeal(effectData, a, damageEv.value or 0, session)
            healEv.value, healEv.cap = healed, cap
    ...
    if (effectData.type == "add_status" or effectData.type == "remove_status") and b then
        ...
        local events = applyMature(effectData, a, b, session, context)
        local changed = effectData.type == "add_status" and findEvent(events, "state_add", b)
        ...
        if changed then
            ...
            if afterMax ~= beforeMax then
                ...
                table.insert(events, maxHpEvent(b, transition, { temporary = true,
                    state = effectData.status or effectData.value }))
                if transition.hpGranted > 0 then
                    table.insert(events, { type = "heal", target = b, value = transition.hpGranted,
                        cap = afterMax, reason = "max_hp_gain" })
                elseif transition.hpClamped > 0 then
                    table.insert(events, { type = "hp_clamp", target = b, value = b.hp,
    ...
    if effectData.type == "maxHp" or (effectData.type == "param_plus" and permanentParam == "maxHp") then
    ...
    return resolved(applyMature(effectData, a, b, session, context), session)
end
