...
// #475 + owner follow-up after #482: the Light layer is now the coherent
// environment-authoring home. Lamp sources remain the active spatial tool;
// legacy Paint/Blur is shelved rather than advertised as a broken hidden-canvas
// workflow, and Vertex Shading is moved beside lighting so its Color Noise
// controls are discoverable without pretending tint is illumination.
(function installStudioLightingAuthoring(root) {
    if (typeof window === 'undefined' || typeof document === 'undefined') return;

    // Start the Three material fidelity patch before the workspace imports the
    // renderer backend. The bundle itself arrives asynchronously after that, so
    // resolved world materials inherit the patched compile contract before they
    // are first presented.
    root.ThestraThreeWorldFidelityReady = import('/js/three-world-fidelity.js')
        .catch(error => {
            console.error('Thestra static world material fidelity failed to initialize:', error);
            return null;
        });
    ...
    function reconcileLightingPalette() {
        ...
        if (intro && intro.tagName === 'P') {
            intro.textContent = 'Author semantic lamp sources here. Vertex Shading below adds environmental color variation independently of illumination.';
        }
        ...
        if (lampHint) {
            ...
            lampHint.textContent = 'Click a cell to add/select a lamp, then use the 3D gizmo to move it. Lamps affect illumination and collision remains unchanged.';
        }
    ...
    reconcileLightingPalette();
    const observer = new MutationObserver(() => {
        if (reconcileLightingPalette()) observer.disconnect();
    });
