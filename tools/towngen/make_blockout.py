...
from build_town import (  # noqa: E402
    SCREENS, screen_lane, lane_y_for, screen_scale, WORLD_H,
    NATIVE_W, LANE_MARGIN_PX)
...
PLATE_SCREENS = (
    "port", "churchyard", "cortico", "market", "quay", "praca", "weaponsmith",
    "alicias_padaria", "pub", "chapel", "house_laura", "house_alicia",
    "lodging",
)
EXTERIORS = ("churchyard", "cortico", "market", "quay", "port")
...
def spec(key):
    ...
    for item in screen["doors"]:
        anchor, label, target, arrival, pixel_x, _src = item[:6]
        y = lane_y_for(pixel_x, scale)
        openings.append({
            ...
            "kind": "street" if any(abs(y - bound) < 0.001 for bound in bounds) else "door",
        })
    return {
        "screen": key,
        "mapId": screen["id"],
        ...
        "compositionMarginPx": LANE_MARGIN_PX,
        "horizonY": 66,
        "player": {"width": PLAYER_W, "height": PLAYER_H,
                   "feetY": ground_y, "headY": ground_y - PLAYER_H},
        "openings": openings,
        "npcs": [{"anchor": a, "pixelX": round(float(px), 3)}
                 for a, _s, _sp, px in screen["npcs"]],
    }
    ...
    for key in PLATE_SCREENS:
        ...
        print("%-11s map %-3d %5dpx  %d doors, %d street exits, %d npcs"
              % (key, s["mapId"], s["plateWidth"], doors, streets,
                 len(s["npcs"])))
    print("wrote %d specs to %s" % (len(PLATE_SCREENS), args.out))
