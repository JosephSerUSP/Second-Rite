...
actual image. The rebased stack now inherits the current `walker.png` from
`main`: 144x48, dimensionally compatible with a 6x1 grid of 24x48 frames.
`frame_width=24, frame_height=48` therefore validates that grid against the real
image before selecting a frame; #852 still does not author, copy, or reinterpret
the sprite itself.