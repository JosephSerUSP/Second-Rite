...
tools/blender/recipes/   St. Maria environment recipes. `interior.py` is the
                         ...
                         A recipe-generated `.blend` under
                         `projects/*/assets/authoring/environments/` remains
                         regenerable scaffold output until its owner adopts or
                         hand-edits it. After adoption it is SOURCE AUTHORITY:
                         edit it directly and never regenerate it. `--force`
                         requires knowing which state the document is in.
