...
# Master Adversarial Gauntlet Automation Driver for Second Gate
...
if os.path.basename(REPO_ROOT) == "gauntlet":
...
def run_blender_render(character: str, action: str, output_dir: str, blend_out: Optional[str] = None):
    ...
    runner_script = os.path.join(REPO_ROOT, "gauntlet", "pipeline", "render_runner.py")
    cmd = [
        BLENDER_EXE,
        "--factory-startup",
        "-b",
...
def build_character_contact_sheets(char_name: str, round_dir: str) -> Dict[str, str]:
    ...
    sheets = {}
    sheets_dir = os.path.join(round_dir, "sheets")
    ...
    if os.path.exists(static_file):
        ...
        sheet_path = os.path.join(sheets_dir, f"{char_name}_static_inspection.png")
        create_static_sheet(
            static_file,
            sheet_path,
    ...
    if idle_files:
        strip_path = os.path.join(sheets_dir, f"{char_name}_idle_strip.png")
        create_animation_strip(
            idle_files,
            strip_path,
    ...
    for d in walk_dirs:
        ...
        if dfiles:
            dir_map[d] = dfiles
    if len(dir_map) == 8:
        grid_path = os.path.join(sheets_dir, f"{char_name}_locomotion_grid.png")
        create_locomotion_grid(
            dir_map,
            grid_path,
    ...
    if gesture_files:
        strip_path = os.path.join(sheets_dir, f"{char_name}_gesture_strip.png")
        create_animation_strip(
            gesture_files,
            strip_path,
...
def run_character_gauntlet_round(
    ...
    4. Submit package to gpt-5.6-luna on xhigh reasoning for adversarial critique.
    ...
    # 2. Authoritative Ground Anchor Stabilization Pass (176px lock)
    from gauntlet.pipeline.sprite_processor import stabilize_round_sprites
    st_count = stabilize_round_sprites(round_dir, target_x=96, target_y=176, alpha_threshold=40)
    print(f"[SpriteProcessor] Stabilized {st_count} frames to exact ground anchor (96, 176).")
    ...
    # 4. Submit to Luna
    harness = LunaHarness()
    print("[LunaHarness] Submitting evaluation package to gpt-5.6-luna (xhigh reasoning)...")
...
def run_ensemble_gauntlet_round(round_idx: int) -> Dict[str, Any]:
    ...
    eval_images = [
        ...
        {"label": "Ensemble Native 1x vs 4x Pixel Inspection", "path": sheets["pixel_inspection"]}
    ]
    ...
    context_prompt = (
        ...
        "Inspect across all core criteria:\n"
    ...
    # 2. Submit to Luna
    harness = LunaHarness()
    print("[LunaHarness] Submitting ensemble evaluation package to gpt-5.6-luna (xhigh reasoning)...")
    luna_result = harness.evaluate_round(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--character", type=str, choices=["celina", "agnes", "gambler", "ensemble"], required=True)
    parser.add_argument("--round", type=int, default=1)
    args = parser.parse_args()

    if args.character == "ensemble":
        run_ensemble_gauntlet_round(args.round)
    else:
        run_character_gauntlet_round(args.character, args.round)
