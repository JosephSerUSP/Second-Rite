...
def create_actor_preview(image_path, camera_obj, *, anchor=(0.0, 0.0, 0.0),
    ...
    # Blender's camera matrix carries a handed view basis: its local +Z axis
    # is the backward ray.  A billboard needs the same right/up axes but the
    # opposite depth axis to form a proper rotation.  Keep this conversion in
    # the generic helper; callers must not derive their own billboard basis.
    actor_basis = camera_obj.matrix_world.to_3x3().copy()
    for row in range(3):
        actor_basis[row][2] *= -1.0
    obj.rotation_quaternion = actor_basis.transposed().to_quaternion().inverted()
    obj["thestra_preview_only"] = True
