...
def create_actor_preview(image_path, camera_obj, *, anchor=(0.0, 0.0, 0.0),
    ...
    # Adopt the camera's own 3x3 basis directly rather than going through
    # to_quaternion(). Thestra's resolved orientation (forward +X, right +Y,
    # up +Z) is a LEFT-handed triple, so the camera basis has determinant -1.
    # A quaternion cannot represent a reflection: to_quaternion() silently
    # discards it and returns a rotation that maps the plane's local +Y to
    # world -Z, hanging the actor downward from its own feet anchor. Copying
    # the basis preserves it, so local +Y is screen-up and local +X is
    # screen-right exactly as the camera resolves them.
    basis = camera_obj.matrix_world.to_3x3()
    for i in range(3):
        column = Vector((basis[0][i], basis[1][i], basis[2][i]))
        if column.length > 0:
            column.normalize()
        for row in range(3):
            basis[row][i] = column[row]
