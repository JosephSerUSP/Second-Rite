 ...
 */
var ThestraSpriteResolutionSemantics;
(function (ThestraSpriteResolutionSemantics) {
    /**
     ...
     */
    ThestraSpriteResolutionSemantics.ASSET_DIRS = [
    ...
    function capitalizedAscii(value) {
        if (value.length === 0)
            return value;
     ...
     */
    function candidatePaths(fileKey) {
        return [
    ...
    ThestraSpriteResolutionSemantics.candidatePaths = candidatePaths;
    /**
     ...
     */
    function buildFileIndex(entries, fileKeyOf) {
        const index = {};
        for (let i = 0; i < entries.length; i++) {
            const entry = entries[i];
            if (!entry || !isPng(entry.name))
                continue;
    ...
    ThestraSpriteResolutionSemantics.buildFileIndex = buildFileIndex;
    /**
     ...
     */
    function probeOrder(fileKey, index) {
        const paths = candidatePaths(fileKey);
        const indexed = index ? index[fileKey.toLowerCase()] : undefined;
        if (indexed)
            paths[paths.length] = indexed.path;
        return paths;
    }
    ThestraSpriteResolutionSemantics.probeOrder = probeOrder;
    /** The indexed entry for a key, or null. Hosts read `stem` for filename tokens. */
    function indexedFor(fileKey, index) {
        if (!index)
            return null;
        const hit = index[fileKey.toLowerCase()];
        return hit === undefined ? null : hit;
    }
    ThestraSpriteResolutionSemantics.indexedFor = indexedFor;
})(ThestraSpriteResolutionSemantics || (ThestraSpriteResolutionSemantics = {}));
