...
function inheritedEnginePath(rtpRoot, revision) {
    return revision
        ? path.resolve(rtpRoot, 'revisions', revision, 'data', 'engine.json')
        : null;
}

function resolveEngineIfPresent(projectDir, systemValue, rtpRoot) {
    const projectPath = path.resolve(projectDir, 'data', 'engine.json');
    const revision = rtp.pinnedRevision(systemValue);
    const inheritedPath = inheritedEnginePath(rtpRoot, revision);
    const present = fs.existsSync(projectPath) || (inheritedPath && fs.existsSync(inheritedPath));
    if (!present) return null;
    return engine.resolve({ projectDir, systemValue, rtpRoot });
}

function hasFragmentedSceneIndex(projectDir) {
    ...
    return Array.isArray(files)
        && files.some(file => typeof file === 'string' && file.toLowerCase().endsWith('.json'));
}
...
function resolveAndMaterialize({ projectDir, runtimeDir, stageDir, rtpRoot, packageContributions }) {
    ...
    const engineRegistry = resolveEngineIfPresent(projectDir, system.value, root);
    const sounds = rtp.sounds({
        projectDir,
        systemValue: system.value,
        rtpRoot: root,
        packageContributions,
    });
    const scenes = hasFragmentedSceneIndex(projectDir)
        ? defaults.scenes({ projectDir, systemValue: system.value, rtpRoot: root })
        : [];
    ...
    if (scenes.length && fs.existsSync(indexPath)) {
        const parsed = JSON.parse(fs.readFileSync(indexPath, 'utf8'));
        const files = (Array.isArray(parsed) ? parsed : parsed.files).slice();
        for (const resource of scenes) {
            if (resource.provider.kind !== 'rtp') continue;
            const filename = path.basename(resource.logicalPath);
            copy(resource.sourcePath, path.join(stageDir, resource.logicalPath));
    ...
    for (const resource of flows) {
        if (resource.provider.kind === 'rtp') copy(resource.sourcePath, path.join(stageDir, resource.logicalPath));
    }
